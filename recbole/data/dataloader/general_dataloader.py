    ...
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        if config['method'] in ['DuoRec_XAUG']:
            ...
            if os.path.exists(inter_sect_dir_file):  # and self.config['dataset'] != 'ml-1m':
                ...
                np.save(inter_sect_dir_file, np.array(intersect_target_index,dtype=object))
                np.save(union_sect_dir_file, np.array(union_target_index,dtype=object))
                self.intersect_target_index = np.array(intersect_target_index,dtype=object)
                self.union_target_index = np.array(union_target_index,dtype=object)
